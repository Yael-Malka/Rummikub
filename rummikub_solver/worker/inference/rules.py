"""Assign numbers/colors to grouped tiles."""
...
CONF_SUGGEST = 0.60   # only propose a fix when the model's own conf is below this
MIN_MELD_LEN = 3      # runs/groups need at least 3 tiles
MAX_GROUP_LEN = 4     # a group has at most 4 (one per color)
JOKER = "joker"
...
def meld_score(series: Sequence[dict]) -> float:
    """
    Return a quality score (0..1) for the best meld fit of `series`.
    Used by the content-based refinement pass in grouping.py.
    """
    if len(series) < 2:
        return 0.0
    run = _fit_run(series)
    ...
    # Penalise sequences that are out of the valid meld-length window: they
    # shouldn't block a split even if their raw score is high.
    if len(series) > 13:
        score *= 0.5
    return score
