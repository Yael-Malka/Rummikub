"""Debug overlays for inference output."""
...
# BGR colors keyed by set validity / status.
STATUS_COLOR = {
    "valid":        (0, 170, 0),     # green
    "review":       (0, 140, 255),   # orange
    "incomplete":   (150, 150, 150), # gray
}
SUGGEST_COLOR = (200, 0, 200)        # magenta for advisory fixes
...
def draw_board(image_bgr, sets: list[dict], meta: dict,
    ...
    """
    Return an annotated copy of the board image. The canvas is auto-upscaled so a
    tile is ~`target_tile_px` tall, keeping labels legible on small/low-res photos.
    Bbox coords in `sets` stay in original pixels (only the drawing is scaled).
    """
    ...
    for s in sets:
        ...
        # Set label above the box (or below if near the top edge).
        text = _label(s)
        ...
        # Per-tile predicted label + advisory suggestion.
        for t in s["tiles"]:
...
def write_state(path, image_name: str, meta: dict,
    ...
    """Serialize the reconstructed board to JSON."""
    state = {
        "image": image_name,
